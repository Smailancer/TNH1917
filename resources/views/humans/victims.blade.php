<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-center text-gray-800 leading-tight">
            {{ __('Humans (Since October 2023)') }}
        </h2>
    </x-slot>
    @include('layouts.HumansNav')

    <div class="container mx-auto px-4 py-8">
        <div class="flex flex-wrap -mx-2 justify-center">
            <!-- Card 1 -->
            <div class="w-full md:w-1/3 lg:w-1/5 px-2 mb-4 relative">
                <div class="bg-white rounded-lg shadow p-4 relative transition-all duration-300 hover:shadow-lg">
                  <img src="https://cdn.vectorstock.com/i/preview-1x/80/91/person-gray-photo-placeholder-little-boy-vector-22808091.jpg" alt="Placeholder" class="rounded-full w-24 h-24 mx-auto object-cover">
                  <h2 class="text-xl font-semibold text-center mt-2">Name Placeholder</h2>
                  <p class="text-center text-gray-600">Birth: YYYY-MM-DD</p>
                  <p class="text-center text-gray-600">Death: YYYY-MM-DD</p>
                  <p class="text-sm text-gray-600 mt-2 text-center">
                    Additional info here.
                  </p>
                  <!-- Black Ribbon Overlay -->
                  <div class="absolute top-0 left-0 w-12 h-12 bg-black" style="clip-path: polygon(0 0, 100% 0, 0 100%);"></div>
                </div>
              </div>

              <!-- Card 2 -->
              <div class="w-full md:w-1/3 lg:w-1/5 px-2 mb-4 relative">
                <div class="bg-white rounded-lg shadow p-4 relative transition-all duration-300 hover:shadow-lg">
                  <img src="https://cdn.vectorstock.com/i/preview-1x/80/91/person-gray-photo-placeholder-little-boy-vector-22808091.jpg" alt="Placeholder" class="rounded-full w-24 h-24 mx-auto object-cover">
                  <h2 class="text-xl font-semibold text-center mt-2">Name Placeholder</h2>
                  <p class="text-center text-gray-600">Birth: YYYY-MM-DD</p>
                  <p class="text-center text-gray-600">Death: YYYY-MM-DD</p>
                  <p class="text-sm text-gray-600 mt-2 text-center">
                    Additional info here.
                  </p>
                  <!-- Black Ribbon Overlay -->
                  <div class="absolute top-0 left-0 w-12 h-12 bg-black" style="clip-path: polygon(0 0, 100% 0, 0 100%);"></div>
                </div>
              </div>

              <!-- Card 3 -->
              <div class="w-full md:w-1/3 lg:w-1/5 px-2 mb-4 relative">
                <div class="bg-white rounded-lg shadow p-4 relative transition-all duration-300 hover:shadow-lg">
                  <img src="https://cdn.vectorstock.com/i/preview-1x/80/91/person-gray-photo-placeholder-little-boy-vector-22808091.jpg" alt="Placeholder" class="rounded-full w-24 h-24 mx-auto object-cover">
                  <h2 class="text-xl font-semibold text-center mt-2">Name Placeholder</h2>
                  <p class="text-center text-gray-600">Birth: YYYY-MM-DD</p>
                  <p class="text-center text-gray-600">Death: YYYY-MM-DD</p>
                  <p class="text-sm text-gray-600 mt-2 text-center">
                    Additional info here.
                  </p>
                  <!-- Black Ribbon Overlay -->
                  <div class="absolute top-0 left-0 w-12 h-12 bg-black" style="clip-path: polygon(0 0, 100% 0, 0 100%);"></div>
                </div>
              </div>

              <!-- Card 4 -->
              <div class="w-full md:w-1/3 lg:w-1/5 px-2 mb-4 relative">
                <div class="bg-white rounded-lg shadow p-4 relative transition-all duration-300 hover:shadow-lg">
                  <img src="https://cdn.vectorstock.com/i/preview-1x/80/91/person-gray-photo-placeholder-little-boy-vector-22808091.jpg" alt="Placeholder" class="rounded-full w-24 h-24 mx-auto object-cover">
                  <h2 class="text-xl font-semibold text-center mt-2">Name Placeholder</h2>
                  <p class="text-center text-gray-600">Birth: YYYY-MM-DD</p>
                  <p class="text-center text-gray-600">Death: YYYY-MM-DD</p>
                  <p class="text-sm text-gray-600 mt-2 text-center">
                    Additional info here.
                  </p>
                  <!-- Black Ribbon Overlay -->
                  <div class="absolute top-0 left-0 w-12 h-12 bg-black" style="clip-path: polygon(0 0, 100% 0, 0 100%);"></div>
                </div>
              </div>

              <!-- Card 5 -->
              <div class="w-full md:w-1/3 lg:w-1/5 px-2 mb-4 relative">
                <div class="bg-white rounded-lg shadow p-4 relative transition-all duration-300 hover:shadow-lg">
                  <img src="https://cdn.vectorstock.com/i/preview-1x/80/91/person-gray-photo-placeholder-little-boy-vector-22808091.jpg" alt="Placeholder" class="rounded-full w-24 h-24 mx-auto object-cover">
                  <h2 class="text-xl font-semibold text-center mt-2">Name Placeholder</h2>
                  <p class="text-center text-gray-600">Birth: YYYY-MM-DD</p>
                  <p class="text-center text-gray-600">Death: YYYY-MM-DD</p>
                  <p class="text-sm text-gray-600 mt-2 text-center">
                    Additional info here.
                  </p>
                  <!-- Black Ribbon Overlay -->
                  <div class="absolute top-0 left-0 w-12 h-12 bg-black" style="clip-path: polygon(0 0, 100% 0, 0 100%);"></div>
                </div>
              </div>
              
              <div class="w-full md:w-1/3 lg:w-1/5 px-2 mb-4 relative">
                <div class="bg-white rounded-lg shadow p-4 relative transition-all duration-300 hover:shadow-lg">
                  <img src="https://cdn.vectorstock.com/i/preview-1x/80/91/person-gray-photo-placeholder-little-boy-vector-22808091.jpg" alt="Placeholder" class="rounded-full w-24 h-24 mx-auto object-cover">
                  <h2 class="text-xl font-semibold text-center mt-2">Name Placeholder</h2>
                  <p class="text-center text-gray-600">Birth: YYYY-MM-DD</p>
                  <p class="text-center text-gray-600">Death: YYYY-MM-DD</p>
                  <p class="text-sm text-gray-600 mt-2 text-center">
                    Additional info here.
                  </p>
                  <!-- Black Ribbon Overlay -->
                  <div class="absolute top-0 left-0 w-12 h-12 bg-black" style="clip-path: polygon(0 0, 100% 0, 0 100%);"></div>
                </div>
              </div>
              
              <div class="w-full md:w-1/3 lg:w-1/5 px-2 mb-4 relative">
                <div class="bg-white rounded-lg shadow p-4 relative transition-all duration-300 hover:shadow-lg">
                  <img src="https://cdn.vectorstock.com/i/preview-1x/80/91/person-gray-photo-placeholder-little-boy-vector-22808091.jpg" alt="Placeholder" class="rounded-full w-24 h-24 mx-auto object-cover">
                  <h2 class="text-xl font-semibold text-center mt-2">Name Placeholder</h2>
                  <p class="text-center text-gray-600">Birth: YYYY-MM-DD</p>
                  <p class="text-center text-gray-600">Death: YYYY-MM-DD</p>
                  <p class="text-sm text-gray-600 mt-2 text-center">
                    Additional info here.
                  </p>
                  <!-- Black Ribbon Overlay -->
                  <div class="absolute top-0 left-0 w-12 h-12 bg-black" style="clip-path: polygon(0 0, 100% 0, 0 100%);"></div>
                </div>
              </div>
              
              <div class="w-full md:w-1/3 lg:w-1/5 px-2 mb-4 relative">
                <div class="bg-white rounded-lg shadow p-4 relative transition-all duration-300 hover:shadow-lg">
                  <img src="https://cdn.vectorstock.com/i/preview-1x/80/91/person-gray-photo-placeholder-little-boy-vector-22808091.jpg" alt="Placeholder" class="rounded-full w-24 h-24 mx-auto object-cover">
                  <h2 class="text-xl font-semibold text-center mt-2">Name Placeholder</h2>
                  <p class="text-center text-gray-600">Birth: YYYY-MM-DD</p>
                  <p class="text-center text-gray-600">Death: YYYY-MM-DD</p>
                  <p class="text-sm text-gray-600 mt-2 text-center">
                    Additional info here.
                  </p>
                  <!-- Black Ribbon Overlay -->
                  <div class="absolute top-0 left-0 w-12 h-12 bg-black" style="clip-path: polygon(0 0, 100% 0, 0 100%);"></div>
                </div>
              </div>
              
              <div class="w-full md:w-1/3 lg:w-1/5 px-2 mb-4 relative">
                <div class="bg-white rounded-lg shadow p-4 relative transition-all duration-300 hover:shadow-lg">
                  <img src="https://cdn.vectorstock.com/i/preview-1x/80/91/person-gray-photo-placeholder-little-boy-vector-22808091.jpg" alt="Placeholder" class="rounded-full w-24 h-24 mx-auto object-cover">
                  <h2 class="text-xl font-semibold text-center mt-2">Name Placeholder</h2>
                  <p class="text-center text-gray-600">Birth: YYYY-MM-DD</p>
                  <p class="text-center text-gray-600">Death: YYYY-MM-DD</p>
                  <p class="text-sm text-gray-600 mt-2 text-center">
                    Additional info here.
                  </p>
                  <!-- Black Ribbon Overlay -->
                  <div class="absolute top-0 left-0 w-12 h-12 bg-black" style="clip-path: polygon(0 0, 100% 0, 0 100%);"></div>
                </div>
              </div>
              
              <div class="w-full md:w-1/3 lg:w-1/5 px-2 mb-4 relative">
                <div class="bg-white rounded-lg shadow p-4 relative transition-all duration-300 hover:shadow-lg">
                  <img src="https://cdn.vectorstock.com/i/preview-1x/80/91/person-gray-photo-placeholder-little-boy-vector-22808091.jpg" alt="Placeholder" class="rounded-full w-24 h-24 mx-auto object-cover">
                  <h2 class="text-xl font-semibold text-center mt-2">Name Placeholder</h2>
                  <p class="text-center text-gray-600">Birth: YYYY-MM-DD</p>
                  <p class="text-center text-gray-600">Death: YYYY-MM-DD</p>
                  <p class="text-sm text-gray-600 mt-2 text-center">
                    Additional info here.
                  </p>
                  <!-- Black Ribbon Overlay -->
                  <div class="absolute top-0 left-0 w-12 h-12 bg-black" style="clip-path: polygon(0 0, 100% 0, 0 100%);"></div>
                </div>
              </div>
              
        </div>
    </div>

</x-app-layout>
