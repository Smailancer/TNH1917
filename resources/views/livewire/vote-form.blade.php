<section id="voting" class="bg-gray dark:bg-gray-900 px-3 mx-2 flex justify-center min-h-screen ">
    <div class="lg:pt-16 lg:pb-24 bg-white m-2 p-3 duration-1000 opacity-70 -inset-px bg-gradient-to-r from-yellow-500 to-yellow-500 rounded-xl group-hover:opacity-100 group-hover:-inset-1 group-hover:duration-200 animate-tilt">
        <form action="{{ route('votes.store') }}" method="POST">
            ...
            <div x-data="{ countryId: null, countryCode: '' }">
                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Choose a country</label>
                ...
                <template x-if="countryCode">
                    <div class="my-4 grid grid-cols place-items-center ">
                        ...
                        <figcaption class="mt-2 text-sm text-center text-gray-500 dark:text-gray-400" x-text="`Map of ${countryCode}`"></figcaption>
                    </div>
                </template>
            </div>
            ...
            <div x-data="{ percentage: 50 }" class="mb-8">
                <div class="relative mb-6">
                    <label for="percentage-range" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Percentage you want to give from the native country</label>
                    <input x-model="percentage" type="range" name="percentage" id="percentage-range" class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-900" min="0" max="100">
            ...
            @foreach ($options as $option)
                <div class="flex items-center my-4">
                    <input wire:model="selectedOptions" type="checkbox" value="{{ $option->id }}" id="option-{{ $option->id }}" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
            ...
            <div class="sm:col-span-2 my-4">
                <label for="notes" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Other Suggestions :</label>
